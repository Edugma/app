...
class StoreBuilder<TKey, TData> {
    ...
    class CacheBuilder<TKey, TData> {
        internal var read: (suspend Store<TKey, TData>.(key: TKey) -> Flow<TData?>) by Delegates.notNull()
        internal var write: (suspend Store<TKey, TData>.(key: TKey, data: TData) -> Unit) by Delegates.notNull()
        internal var expiresIn: Duration by Delegates.notNull()

        fun reader(read: suspend Store<TKey, TData>.(key: TKey) -> Flow<TData?>) {
            this.read = read
        }
